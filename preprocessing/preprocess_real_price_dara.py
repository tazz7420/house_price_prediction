...
def process(input_citycode, output_cityname):
    path_text = r'../rawdata/real_price/'
    path = os.listdir(path_text)
    n = 0
    for filename in path:
        try:
            if (filename[-3:] == 'csv') & (filename.split('_')[2] == input_citycode):
                if n == 0:
    ...
    pre_df = df1.loc[filt,['土地位置建物門牌','鄉鎮市區','交易標的','交易年月日','建築完成年月','建物型態','建物現況格局-房','建物現況格局-廳','建物現況格局-衛','車位類別','電梯','單價元平方公尺']]
    ...
    filt2 = (pre_df['屋齡'] >= 0)
    pre_df = pre_df.loc[filt2,['交易年份','土地位置建物門牌','鄉鎮市區','屋齡','交易標的','交易年月日','建築完成年月','建物型態','建物現況格局-房','建物現況格局-廳','建物現況格局-衛','車位類別','電梯','單價元平方公尺']]
